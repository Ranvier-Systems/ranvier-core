// Chassis Configuration Schema
//
// Pure infrastructure configuration structs with zero product/LLM coupling.
// These configs are candidates for extraction into a shared chassis library
// (libseastar-chassis) that multiple products can depend on.
//
// Product-specific configs (RoutingConfig, AssetsConfig) remain in config_schema.hpp.
// The top-level RanvierConfig (which composes both) also remains in config_schema.hpp.
//
// For the full config facade, include config.hpp.

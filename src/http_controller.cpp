...
RequestIntent HttpController::classify_intent(
    ...
    for (const auto& field : _config.intent_fim_fields) {
        // Search for "field_name" pattern (quoted JSON key)
        // Build a small search pattern: "\"field_name\""
        // We look for the quoted key to reduce false positives
    ...
    while (system_pos != std::string_view::npos) {
        ...
        for (const auto& keyword : _config.intent_edit_system_keywords) {
        ...
        for (const auto& tag : _config.intent_edit_tag_patterns) {

1. Ref doc/claude-context.md for the "No Locks/Async Only" rules.
2. DO NOT read the full /doc or /assets folders.
3. Run /compact if the conversation exceeds 4 turns.

I am STARTING A PLANNING SESSION for the following feature:
<INSERT_FEATURE_DESCRIPTION>

TASK:
- Break this feature down into atomic, independent steps.
- Identify which of the 33 files will be affected.
- OUTPUT: A Markdown list of tasks for docs/TODO.md (Status: [ ] Pending).

Do not write implementation code yet. Focus only on the roadmap.

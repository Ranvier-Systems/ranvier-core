...
2. **Telemetry sink** (optional) — if `telemetry_sink.enabled`, the controller
   resolves the selected backend's labels via
   `RouterService::telemetry_labels()` (one shard-local lookup yielding
   `backend_type`, `hardware_label`, and `model_family`) and calls
   `TelemetryService::record_outcome()`, bucketing by
   `(model_family, backend_type, hardware_label, workload_pattern)`. Shard-local,
   off the request future; when disabled, a single null-check branch. See

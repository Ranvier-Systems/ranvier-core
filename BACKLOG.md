...
- [x] **[P3] Unblock Seastar-dependent fuzzing** — Completed 2026-05-14. The build-config reshape landed across PRs #482/#484/#486 (Dockerfile.base.default-alloc + sed-patched IN_LIST so SEASTAR_DEFAULT_ALLOCATOR propagates through Seastar's PUBLIC compile-definitions to libseastar's memory.cc; consumer recipe in Dockerfile.fuzz / Dockerfile.sanitize headers; Makefile detection + alias target; fuzz-tests.yml job). A 30-min local fuzz pass against `ranvier-fuzz-default-alloc` ran clean — 21,864,631 executions at 12,140 execs/s, 967 new corpus units, peak RSS 698 MB, no `crash-*` reproducer. H10, M11, M12 promoted to MITIGATED-BY-FUZZ in `docs/audits/request-lifecycle-crash-audit.md` under the "Fuzz update (2026-05-14)" addendum. CI trigger flipped from `workflow_dispatch`-only to the post-Docker-Publish `workflow_run` shape used by the main `fuzz-tests` job, and `continue-on-error: true` dropped so a future smoke-time regression fails CI. The empirical fix-up history (configure.py strict argparse, buildx docker-container driver isolation, no Seastar_USE_DEFAULT_ALLOCATOR cache variable) is preserved in the §18 P3 sub-bullets below so the next reader doesn't re-tread the dead ends. **Spinoff still open:** publish `ranvier-base-default-alloc` to GHCR via `docker-publish.yml` so the default-alloc fuzz/sanitize jobs collapse to single-stage builds with full GHA caching — tracked as a sub-bullet in the §18 P3 closure record. **§18 P2** (the ASan/UBSan unit-test sweep) is the parallel unblock and remains open against the same shared base.

- [ ] **[P3-spinoff] Publish `ranvier-base-default-alloc` to GHCR**
  _Where:_ `.github/workflows/docker-publish.yml`, `.github/workflows/fuzz-tests.yml` (the `stream-parser-fuzz-default-alloc` job), `.github/workflows/sanitizer-tests.yml`.
  _Justification:_ The §18 P3 closure left one workaround in place — the `stream-parser-fuzz-default-alloc` job runs `setup-buildx-action` with `driver: docker` and skips GHA caching, because `docker-container` driver buildx instances can't see the local docker daemon's image store for the chained `FROM ranvier-base-default-alloc:latest` lookup. As a result every dispatch pays the full ~30-min Seastar rebuild. Publishing the image to GHCR (same shape as `ranvier-base`) lets both the fuzz job and the §18 P2 sanitizer-tests workflow collapse to single-stage builds with the existing `BASE_IMAGE` build-arg, restoring full GHA caching and removing the docker-driver workaround.
  _Approach:_ Extend `docker-publish.yml` to build `Dockerfile.base.default-alloc` on top of the cached `ranvier-base` and push it to `ghcr.io/ranvier-systems/ranvier-base-default-alloc:latest`. Then in `fuzz-tests.yml`'s `stream-parser-fuzz-default-alloc` job, drop the `driver: docker` setting, drop the `Build default-allocator base image` step, and change the fuzz build's `BASE_IMAGE` to `ghcr.io/ranvier-systems/ranvier-base-default-alloc:latest`. Mirror in `sanitizer-tests.yml` (same one-line `BASE_IMAGE` override).
  _Complexity:_ Low (Docker Publish + workflow tweaks).

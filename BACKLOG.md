...
- [ ] **[P1] Wire up `stop_metrics()` (Rule #6 / Rule #13)**
  ...
  _Approach:_ Mirror the `cleanup_shard_load_metrics()` step landed in the entry above — `seastar::smp::invoke_on_all([] { stop_metrics(); })` inside `Application::stop_services()`, after the metrics HTTP server has stopped (step 3, `stop_servers()`) so no scrape can race the deregistration. Verify ordering against any other consumer that touches `metrics()` on the shutdown path; the controller and load balancer both record metrics during request flow, so this step belongs after `_load_balancer.stop()` at minimum, and after the `cleanup_shard_load_metrics()` step is fine since they are independent.
  _Risk:_ Deregistering Prometheus lambdas while a scrape is in flight could read freed state. Mitigated by the fact that the metrics HTTP server is stopped in step 3 well before this step, but worth confirming the Seastar HTTP server's `stop()` semantics actually drain in-flight handlers before returning. Add a test if possible.
  _Complexity:_ Low (one new step), but the scrape-race analysis bumps it above trivial.

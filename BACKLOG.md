...
- [ ] **Log exceptions in `BasicConnectionBundle::close()` instead of silently discarding them**
...
- [ ] **Remove `const_cast` in `evict_oldest_global()` by storing the iterator or address by value**
...
- [ ] **Replace manual `_total_idle_connections` tracking with a derived or RAII-guarded counter**
...
- [ ] **Avoid O(B) scan in `evict_oldest_global()` by maintaining a sorted eviction index**
...
- [ ] **Switch `_pools` from `std::unordered_map` to `absl::flat_hash_map`**
...
- [ ] **Make `BasicConnectionBundle::close()` safe for direct callers or mark it private**
...
- [ ] **Track connection reuse ratio with `connections_created` and `connections_reused` stats**

...
- [ ] **Gate-protect the route prune `parallel_for_each` futures instead of discarding them**
...
- [ ] **Add a MAX_PEERS bound to the peer table and DNS discovery insertion paths**
...
- [ ] **Remove dead `update_quorum_state()` or unify with `check_quorum()`**
...
- [ ] **Extend the gate holder lifetime in `process_retries()` to cover the send futures**
...
- [ ] **Replace `std::find()` on `_peer_addresses` vector with an `absl::flat_hash_set` for O(1) peer validation**
...
- [ ] **Serialize the route packet once and reuse across peers, stamping only the per-peer sequence number**
...
- [ ] **Extract route prune dispatch into a `broadcast_prune(BackendId)` helper method**
...
- [ ] **Remove empty catch-rethrow blocks in `GossipService::start()` and `GossipTransport::start()`**

...
- [ ] **[P2] F2: Abseil ASan/headers vs installed-library ABI mismatch (`raw_hash_set` `EmptyGeneration()` returns nullptr)** — *Fix verified, pending merge.* Fix landed on branch `wspires-20260516-sanitize-f2-abseil-rebuild` (option (a) from the shape discussion below: clang+ASan rebuild of Abseil at tag `20230802.0`, installed under `/usr/local` to shadow the GCC-built distro shared library; same instrumentation flags as `CMakeLists.txt:1222` `_san_flags`; `BUILD_SHARED_LIBS=ON` to preserve the distro's shared-library shape; pinned version = pure instrumentation rebuild, no version drift). 2026-05-16 dispatch on that branch (`sanitize-log-2026-05-16-2.txt`) confirmed all 325 Abseil-rooted `Subprocess aborted` cases flipped to Passed and the suite wall-clock dropped 5.7×; see _Empirical signal follow-up 8_ above. Closes for main once the branch merges.
...
- [ ] **[P2] F3: `RateLimiterConcurrencyTest.IndependentLimitersNoInterference` races wall-clock refill window under ASan slowdown**

...
// =============================================================================
// Cross-Shard Load Synchronization Tests
// =============================================================================
// Tests for the cross-shard load sync feature (BACKLOG 20.11).
// These test the shard-local logic: config propagation, get_backend_load()
// including cross-shard hints, and the effect on routing decisions.
// The actual periodic timer and SMP broadcast require a reactor; we test
// the data structures and merge logic via the testing helpers.

class CrossShardLoadSyncTest : public ::testing::Test {
protected:
    RoutingConfig cfg_;
    std::unique_ptr<RouterService> router_;
    ...
    void SetUp() override {
        ...
        cfg_.block_alignment = 1;
        cfg_.backend_drain_timeout = std::chrono::seconds(2);
        cfg_.load_aware_routing = false;  // Control load-aware separately
        cfg_.hash_strategy = RoutingConfig::HashStrategy::JUMP;
        // Enable cross-shard load sync
        cfg_.cross_shard_load_sync = true;
        cfg_.cross_shard_load_sync_interval = std::chrono::milliseconds(5);
...
TEST_F(CrossShardLoadSyncTest, GetBackendLoadReturnsLocalOnlyWhenSyncDisabled) {
    // Disable cross-shard sync, verify get_backend_load returns only local
    cfg_.cross_shard_load_sync = false;
    router_.reset();
    RouterService::reset_shard_state_for_testing(nullptr);
    router_ = std::make_unique<RouterService>(cfg_);
...
TEST_F(CrossShardLoadSyncTest, GetBackendLoadIncludesCrossShardHints) {
    // This test verifies that when cross_shard_load_sync is enabled and
    // cross_shard_load has entries, get_backend_load() returns the sum.
    // Since we can't call apply_load_snapshot directly (it's static in the .cpp),
    // we test via the public interface: when cross_shard_load_sync is enabled
    // but no snapshots have been received, get_backend_load returns local only.
    RouterService::register_backend_for_testing(1, make_addr("10.0.0.1", 8080));

    BackendRequestGuard guard(1);
    // With sync enabled but no snapshots received, should still be local only
    EXPECT_EQ(get_backend_load(1), 1u);
}

TEST_F(CrossShardLoadSyncTest, ConfigPropagatedToShardState) {
    // Verify that the cross-shard sync config is properly stored.
    // If the config wasn't propagated, cross_shard_load_sync would be false
    // and get_backend_load would skip the cross-shard lookup entirely.
    // We verify this indirectly: with sync=true, the function attempts the
    // cross_shard_load lookup (even if empty); with sync=false, it doesn't.
    RouterService::register_backend_for_testing(1, make_addr("10.0.0.1", 8080));

    BackendRequestGuard guard(1);
    // Load should be 1 (local) + 0 (no cross-shard data yet) = 1
    EXPECT_EQ(get_backend_load(1), 1u);
}

TEST_F(CrossShardLoadSyncTest, LoadAwareRoutingWithCrossShardHints) {
    ...
    // With no load anywhere, routing should work normally
    auto result = router_->get_backend_for_prefix(tokens);
    ASSERT_TRUE(result.backend_id.has_value());
}

TEST_F(CrossShardLoadSyncTest, GetBackendLoadReturnsZeroForUnknownWithSync) {
    // Unknown backends should return 0 even with sync enabled
    EXPECT_EQ(get_backend_load(999), 0u);
}

TEST_F(CrossShardLoadSyncTest, MultipleGuardsWithSyncEnabled) {
    // Multiple guards should still correctly track local load with sync enabled
    RouterService::register_backend_for_testing(1, make_addr("10.0.0.1", 8080));
    RouterService::register_backend_for_testing(2, make_addr("10.0.0.2", 8080));

    BackendRequestGuard g1(1);
    BackendRequestGuard g2(1);
    BackendRequestGuard g3(2);

    EXPECT_EQ(get_backend_load(1), 2u);
    EXPECT_EQ(get_backend_load(2), 1u);
}

TEST_F(CrossShardLoadSyncTest, LeastLoadedBackendWithSyncEnabled) {
    // get_least_loaded_backend should work correctly with sync enabled
    RouterService::register_backend_for_testing(1, make_addr("10.0.0.1", 8080));
    RouterService::register_backend_for_testing(2, make_addr("10.0.0.2", 8080));
